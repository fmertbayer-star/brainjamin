...
export const submitDuelAnswers = onCall<SubmitDuelAnswersRequest>(
  ...
  async (request) => {
    ...
    const willComplete =
      (isPlayer1 && status === "player2_done") ||
      (!isPlayer1 && status === "player1_done");
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      if (winnerId === null) {
        tx.set(p1Ref, {xp: FieldValue.increment(25)}, {merge: true});
        tx.set(p2Ref, {xp: FieldValue.increment(25)}, {merge: true});
        callerXpAwarded = 25;
      } else if (winnerId === txPlayer1Id) {
        tx.set(p1Ref, {xp: FieldValue.increment(50)}, {merge: true});
        tx.set(p2Ref, {xp: FieldValue.increment(10)}, {merge: true});
        callerXpAwarded = isPlayer1 ? 50 : 10;
      } else {
        tx.set(p1Ref, {xp: FieldValue.increment(10)}, {merge: true});
        tx.set(p2Ref, {xp: FieldValue.increment(50)}, {merge: true});
    ...
    logger.info("submitDuelAnswers submitted", {uid, duelId, correctCount, willComplete});
    return result;
  }
);
